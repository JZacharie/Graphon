...
impl MailSortingPipeline {
    ...
    pub async fn run(&self) -> Result<(), GraphonError> {
        ...
        for rule in rules {
            ...
            let target_emails = self.gmail_client.fetch_emails_by_query(&rule.query).await?;
            let now = Utc::now();
            for email in target_emails {
                ...
                if age.num_days() >= rule.duration_days {
                    ...
                    self.gmail_client.trash_email(&email.id).await?;
                }
            }
        }

        info!("Mail sorting pipeline completed successfully.");
        Ok(())
    }
}
